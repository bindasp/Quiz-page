...
const EditQuiz: React.FC = () => {
  ...
  const handleCheckboxClick = (questionIndex: number, answerIndex: number) => {
    if (!form.values.questions[questionIndex].answers[answerIndex].isCorrect)
      form.values.questions[questionIndex].answers[answerIndex].isCorrect =
        true;
    else if (
      form.values.questions[questionIndex].answers[answerIndex].isCorrect
    )
    ...
    <Stack gap="md">
      <form onSubmit={form.onSubmit(handleSubmit)}>
        ...
        {form.values.questions &&
          form.values.questions.map((question, questionIndex) => (
            <Paper
              mt={10}
              key={questionIndex}
              withBorder={true}
              ...
              {question.answers &&
                question.answers.map((answer, answerIndex) => (
                  ...
                  >
                    ...
                    <Checkbox
                      m={'auto'}
                      defaultChecked={question.answers[answerIndex].isCorrect}
                      onChange={() =>
        ...
        >
          <Button style={{ width: '20%' }} onClick={handleAddQuestion}>
            Dodaj pytanie
          </Button>
          <Button type="submit" style={{ width: '20%', margin: 'auto' }}>
            Zapisz quiz
          </Button>
          <Button style={{ width: '20%' }} onClick={handleDelete}>
            Usuń Quiz
          </Button>
        </div>
      </form>
