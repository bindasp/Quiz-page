  ...
  return (
    <div style={{ width: '100%' }}>
            ...
            >
              ...
              <p>
                ...
                {attempt.completion_time
                  ? new Date(attempt.completion_time).toLocaleString()
                  : 'Nie ukończono'}
              </p>
              <Button size="sm" color="emerald-green">
                Zobacz szczegóły
              </Button>
            </div>
          ))}
      ...
      <Modal
        ...
        title={
          <Title order={3}>
            {selectedAttempt?.test_name} - Szczegóły podejścia
          </Title>
        }
        size="lg"
      >
          ...
          <div>
            <Group justify="space-between" mb="md">
              <div>
                ...
                <Text>
                  {new Date(selectedAttempt.start_time).toLocaleString()}
                </Text>
              </div>
              <div>
              ...
              <div>
                ...
                <Badge
                  ...
                  color={
                    ...
                    Array.from(
                      new Set(
                        selectedAttempt.answers.map((a) => a.question_number),
                      ),
                    ).length
                      ? 'green'
                      : 'yellow'
                ...
                >
                  {Number.isInteger(Number(selectedAttempt.score))
                    ? Math.floor(Number(selectedAttempt.score))
                    : Number(selectedAttempt.score).toFixed(1)}
                  /
                  {
                    Array.from(
                      new Set(
            ...
            {Array.from(
              new Set(selectedAttempt.answers.map((a) => a.question_number)),
            )
              .sort((a, b) => a - b)
              .map((questionNumber) => {
                const questionAnswers = selectedAttempt.answers.filter(
                ...
                return (
                  <Paper key={questionNumber} p="md" mb="md" withBorder>
                    <Text fw={700} mb="xs">
                      Pytanie {questionNumber}: {questionContent}
                    </Text>
